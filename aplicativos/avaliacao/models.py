...
class Avaliacao(models.Model):
    ...
    def clean(self):
        if self.aluno_id and self.competencia_id and self.aluno.ciclo != self.competencia.ciclo:
            raise ValidationError("Aluno e competência devem pertencer ao mesmo ciclo.")
        if not any([self.conhecimentos, self.habilidades, self.atitudes]):
            raise ValidationError("Pelo menos um dos eixos avaliados deve ser marcado.")
